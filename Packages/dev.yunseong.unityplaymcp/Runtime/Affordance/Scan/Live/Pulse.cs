...
namespace Artel.Affordances.Live
{
    ...
    public sealed class Pulse : MonoBehaviour
    {
        ...
        /// 두 판독에 대해 한 번도 움직이지 않았고, 게이트는 뒤 판독을 바뀌지 않은 것으로 쥔다. 그런 값을 봐야 하는 사람은
        /// Project Settings 의 Unity Play MCP 화면에서 간격을 줄이고 그만큼의 읽기 값을 치른다 —
        ...
        /// <remarks>
        /// 읽기와 전달이 두 속도인 것은 일부러다. 기본값에서는 둘이 초당 하나로 같아서 판독 하나가 곧 메시지 하나지만,
        /// 간격을 줄인 프로젝트에서는 다시 갈라진다. 그때 소켓을 지키는 것이 이 값이다: 게임당 초당 작은 메시지 여럿을 나르는
        /// 소켓이 그 판독들을 1초에 한 번 모아 나르는 소켓보다 비싸다.
        ///
        /// 나가는 길에 아무것도 병합하지 않는다. 판독 다섯은 판독 다섯으로 도착하고 각자 몇 번째인지와 어느 프레임에 찍혔는지를
        /// 그대로 말한다. 독자가 그 수열로 1초를 복원하기 때문이다. 배치는 그것들이 *언제 떠나는가* 이지 *무엇인가* 가 아니다.
        /// </remarks>
