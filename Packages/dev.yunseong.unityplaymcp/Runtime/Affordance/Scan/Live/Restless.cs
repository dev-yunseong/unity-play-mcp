...
namespace UnityPlayMcp.Affordances.Live
{
    ...
    /// </summary>
    /// <remarks>
    /// 변화 게이트는 pulse 전체를 비교하므로, 결코 반복되지 않는 값은 매 박자마다 그것을 열어젖히고 게이트는 아무 뜻도
    /// 없어진다 — 모든 pulse 를 받은 독자는 그중 무엇이 소식이었는지를 스스로 알아내야 하는데, 그것이 게이트가 하라고
    /// 존재하는 일이다.
    ///
    ...
    /// 이미 보낸 숫자로 되읽히므로 pulse 가 일치하고 게이트는 닫힌 채로 있다. 일단 가고 나면 새 숫자를 취해 그것이 다음
    ...
    internal sealed class Restless
    {
        ...
        /// 아니라 정확히 같기 때문이다.
        ///
        /// 그것이 맞는지는 논쟁이 아니라 측정의 문제다: pulse 가 거의 다 나가는 실행은 어떤 조건도 언급하지 않는 이유로 움직이는
        /// 값을 가진 것이고, pulse 가 그것이 무엇이었는지 말해 준다.
        ///
