...
namespace UnityPlayMcp.Protocol.Dto
{
    public sealed class ActionResultMessage
    {
        ...
        /// </summary>
        /// <remarks>
        /// pulse 는 자기가 언제 잡혔는지를 말하는데(<c>LiveState</c> 가 <c>frame</c> 을 싣는다) 이 답은 말하지
        /// 않았다. 그래서 받는 쪽이 어떤 pulse 가 이 액션 이후의 것인지 가릴 방법이 없었고, 시간으로 어림잡았다 —
        /// 액션을 보내고 잠시 안에 pulse 가 하나 더 오면 그것을 결과로 쳤다.
        ///
        ...
        ///
        /// <see cref="UnityEngine.Time.frameCount"/> 다 — pulse 가 쓰는 바로 그 시계. 다른 시계였다면 이 값과
        /// pulse 의 값을 견주는 일이 뜻을 잃는다.
        ///
