...
namespace UnityPlayMcp
{
    ...
    /// </summary>
    /// <remarks>
    /// 새 소켓을 열지 않는다. pulse 는 게임이 도는 동안만 흐르고 그 창은 이미 연결이 서 있는
    ...
    internal sealed class WebSocketPulseSink : IPulseSink
    {
        ...
        /// 표시하고 다음 pulse 를 전량으로 만드는데, 조용히 성공한 척하면 그 복구가 돌지 않아
        ...
        /// </summary>
        /// <remarks>
        /// 문자열을 그대로 이어 붙이는 것은 pulse 가 <c>{"schema":</c> 로 시작한다는 것이
        /// <see cref="LiveState"/> 안에서 한 자리에 적혀 있기 때문이다. 파싱해서 다시
        /// 직렬화하면 전량 pulse 18 KB 를 초당 한 번 두 번씩 훑게 되고, 이 패키지가 JSON 을
        /// 손으로 쓰는 이유가 바로 그 값을 치르지 않으려는 것이다.
        ///
        /// <c>type</c> 은 orchestration 의 핸들러 등록제가 읽는 값이고, <c>id</c> 는 같은
        /// pulse 의 재전송이 로그에서 한 번만 적재되게 하는 값이다.
        /// </remarks>
