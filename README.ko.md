...
바꾸려면 **Edit > Project Settings > Unity Play MCP**를 열고 **Reading interval (s)**를 지정합니다. 고른 값은 이 project에 대해 계속 기억됩니다. 0.02초부터 10초까지 넣을 수 있고, 범위를 벗어난 값은 가까운 끝으로 잘리며, 숫자로 읽을 수 없는 값은 기본값 1초가 됩니다. 바꾼 간격은 이미 돌고 있는 감시가 아니라 다음 감시가 시작할 때부터 적용됩니다.

간격을 줄이거나 늘릴 때의 득실은 다음과 같습니다.

- **짧게** 하면 두 번의 읽기 사이에 올라갔다 내려오는 값을 잡을 수 있습니다. 1초에서는 그 1초 안에 올라갔다 내려온 counter가 한 번도 움직이지 않은 것으로 보입니다.
- **길게** 하면 game이 치르는 값이 줄어듭니다. 읽기는 game이 도는 동안 감시 대상 member를 훑으므로, 0.1초는 그것을 초당 열 번 한다는 뜻입니다. Play Mode가 눈에 띄게 느려질 만큼이고, 그러면 agent가 보는 화면이 game이 실제로 어떻게 도는지를 말하지 못합니다.

1초보다 빠르게 움직이는 값을 쫓을 때에만 줄이고, 끝나면 되돌리세요.
